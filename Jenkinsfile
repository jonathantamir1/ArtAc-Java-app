pipeline {
    ...
    environment {
        DOCKER_IMAGE = "${env.DOCKER_IMAGE ?: 'your-dockerhub-username/chatbot-app'}"
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                script { env.GIT_COMMIT_SHORT = readFile('.git-commit').trim() }
            }
        }

        stage('Build & Test') {
            steps {
                // Build and test using a Maven 17 container to avoid relying on Jenkins tool config
                sh '''docker run --rm \
                    -v "$PWD":/workspace \
                    -v "$HOME/.m2":/root/.m2 \
                    -w /workspace \
